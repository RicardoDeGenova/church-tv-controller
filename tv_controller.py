...
from config import INSIDE_TVS, OUTSIDE_TVS
...
class TVIndicator:
    def __init__(self, parent: tk.Frame, name: str, ip: str, row: int):
        self.name = name
        self.ip = ip
...
class ChurchTVController:
    ...
    def build_ui(self) -> None:
        ...
        self.inside_on_btn = self.create_button(
            ...
            "Inside ON",
            lambda: self.turn_on_group(INSIDE_TVS, "inside")
        )
        ...
        self.inside_off_btn = self.create_button(
            inside_buttons_inner,
            "Inside OFF",
            lambda: self.turn_off_group(INSIDE_TVS, "inside")
        )
        ...
        self.outside_on_btn = self.create_button(
            ...
            "Outside ON",
            lambda: self.turn_on_group(OUTSIDE_TVS, "outside")
        )
        ...
        self.outside_off_btn = self.create_button(
            outside_buttons_inner,
            "Outside OFF",
            lambda: self.turn_off_group(OUTSIDE_TVS, "outside")
        )
    ...
    def create_tv_group_frame(
        ...
        for row, (name, ip) in enumerate(tv_list):
            indicator_key = f"{group_prefix}_{name}"
            indicator = TVIndicator(inner_frame, name, ip, row)
    ...
    def get_indicator_key(self, name: str, ip: str) -> str | None:
        for key, indicator in self.indicators.items():
            if indicator.name == name and indicator.ip == ip:
                return key
        return None
    
    def set_indicators_connecting(self, tv_list: list[tuple[str, str]]) -> None:
        for name, ip in tv_list:
            key = self.get_indicator_key(name, ip)
    ...
    def turn_on_group(self, tv_list: list[tuple[str, str]], group_name: str) -> None:
    ...
    def turn_off_group(self, tv_list: list[tuple[str, str]], group_name: str) -> None:
        self.run_threaded_operation(
            tv_list,
            turn_off_single_tv,
    ...
    def turn_all_on(self) -> None:
        all_tvs = INSIDE_TVS + OUTSIDE_TVS
        self.run_threaded_operation(
            all_tvs,
            turn_on_single_tv,
            "Turning on all TVs...",
